<template>
  <div class="container">
    <h2>出库管理</h2>
    <el-button type="primary" @click="showDialog = true">新增出库单</el-button>
    <el-table :data="pagedData" style="margin-top: 20px">
      ...
      <el-table-column label="操作" width="120">
        <template #default="{ row }">
          <el-button v-if="row.status === 'PENDING'" @click="confirm(row.id)" size="small" type="primary">确认出库</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="showDialog" title="新增出库单" width="800px">
...
import { getOutboundOrders, createOutboundOrder, confirmOutboundOrder } from '../api/outbound'
...
const form = ref({ warehouseId: null, operator: '', recipient: '', remark: '', details: [] })
...
const handleSubmit = async () => {
  form.value.outboundDate = new Date().toISOString()
